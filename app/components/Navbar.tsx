...
type NavItem = ScrollNavItem | PageNavItem;
...
const navItems: NavItem[] = [
  ...
  { label: "Leadership", href: "#leadership", type: "scroll" },
  { label: "Programmes", href: "#academic-portfolio", type: "scroll" },
  { label: "Application Process", href: "#application-process", type: "scroll" },
  { label: "Contact", href: "#official-contacts", type: "scroll" },

  // // NEW PAGE LINK
  // { label: "Transcript Payment", href: "/transcript-payment", type: "link" },
];
...
export const Navbar: React.FC = () => {
  ...
  const handleScrollNav = (href: `#${string}`) => {
    ...
    // if user is not on homepage, go home first
    if (pathname !== "/") {
      router.push("/");
      setTimeout(() => {
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className="text-gray-700 hover:text-[#003366] transition-colors font-semibold text-sm"
              >
                {item.label}
              </Link>
            );
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  className="text-gray-700 hover:text-[#003366] text-lg font-semibold border-b border-gray-50 pb-2"
                >
                  {item.label}
                </Link>
              );
